...
export const getVaultAllUnderlyingAssetsByBlock = async (contract: ethers.Contract, blockNumber: number): Promise<{[token: string]: bigint}> => {
    try {
        const underlyingAssets = await contract.vaultAllUnderlyingAssets({blockTag: blockNumber});
        const assets: {[token: string]: bigint} = {};
        underlyingAssets.forEach((asset: any) => {
            assets[asset.token] = BigInt(asset.amount.toString());
        });
        return assets;
    } catch (error) {
